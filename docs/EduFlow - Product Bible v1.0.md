...
| INV-16 | De datum van een documentatie ligt niet meer dan zeven dagen in de toekomst (B-70) en niet vóór 2015-08-01. Dat hij ook niet vóór het oudste schooljaar in de opslag ligt, bewaakt `DocumentationService`. | Je documenteert wat gebeurd is. Een datum ver in de toekomst breekt sortering, filters en de reeksvolgorde, en is bijna altijd een typefout. | Zod voor de twee absolute grenzen, `DocumentationService` voor de schooljaargrens | Opslaan faalt met de melding dat de datum niet klopt; het veld krijgt de nadruk. |
...
| Weekonderdeel | `WeekPatternLine` | les, lesuur, blok, vak, activiteit, afspraak, `Lesson`, `Slot` |
...
| O-09 | Veldtabel voor `settings` | §8.3.14 somt zes onderwerpen op in gewone taal, zonder veldnamen en zonder typen, terwijl INV-49 eist dat het record altijd bestaat | vóór stap 4 |
| O-10 | De vorm van een agenda-item | INV-30 eist onvoorwaardelijk een begin en een einde, INV-31 eist een typeunie waarvan de hele-dag-variant géén tijden heeft, en §6.2.2 geeft één vorm met `start`, `end` en een losse `allDay`. Die drie zijn niet te verenigen; `calendarEvents` blijft daarop geblokkeerd | vóór stap 4 |
| O-11 | Botsingsregel voor agenda en basisweek | §8.1.4 en §8.10 zeggen "`rev` is leidend, `updatedAt` de tiebreaker"; de tabel in §8.7 zegt voor `calendarEvents` "hoogste `updatedAt` wint". Voor `weekPatterns` en `weekPatternOverrides` moet één regel gelden | vóór stap 4 |
