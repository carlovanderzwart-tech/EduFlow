...
> **Lees dit als momentopname, niet als de stand van vandaag.** Dit stuk beschrijft de
> repository zoals hij op 11 augustus was, en dat is precies wat stap 1 vraagt: een
> nulmeting vóór er iets verschuift. Wat er sindsdien mee gedaan is:
>
> | Wat hier staat | Sindsdien |
> |---|---|
> | Drie toetsen rood in `dr-44.test.ts` (§0.1) | Opgelost — de lintregel is weer geregistreerd (stap 0, PR #40) |
> | `pnpm lint`: 48 fouten | 0 fouten; alle zones van `import/no-restricted-paths` staan op `error` |
> | De verplaatsingen uit §5 | Stappen 1 t/m 5 uitgevoerd in PR #40 |
> | 138 bestanden in `src/` | Meer, door D01 t/m D04 |
>
> De volgorde uit §5 en de overtredingen uit §3 zijn dus geschiedenis, geen werklijst.
> Het stuk blijft staan omdat de nulmeting eronder ligt: zonder deze meting is later niet
> na te gaan wat D00 heeft veranderd en wat er al stuk was.
